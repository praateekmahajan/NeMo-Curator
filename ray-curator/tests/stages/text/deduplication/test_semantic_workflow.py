...
from ray_curator.stages.text.deduplication.semantic import TextSemanticDeduplicationWorkflow
...
def create_data_without_duplicates(input_dir: Path) -> pd.DataFrame:
    ...
    df = pd.DataFrame({
            ...
            "text": [
                "A test string",
                "NVIDIA is a company that makes GPUs",
...
class TestTextSemanticDeduplicationWorkflow:
    ...
    def test_semantic_dedup_with_duplicates_and_removal(self):
