...
class CudfLimitAwareFilePartitioningStage(ProcessingStage[_EmptyTask, FileGroupTask]):
    ...
    def process(self, _: _EmptyTask) -> list[FileGroupTask]:
        ...
        if self.filetype == "parquet":
            partitioned_groups = self._break_parquet_files_into_groups(self.embedding_dim, files, self.storage_options)
        else:
            logger.warning(
                f"For {self.filetype} input format, you might run into cudf limits. Consider using a parquet file."
            )
            # For non-parquet files, create individual partitions (one file per group)
            partitioned_groups = [[file] for file in files]
        ...
        for i, file_groups in enumerate(partitioned_groups):
            kmeans_task = FileGroupTask(
                task_id=f"kmeans_partition_{i}",
                dataset_name=dataset_name,
                data=file_groups,
            )
            kmeans_task._metadata["filetype"] = self.filetype
    ...
    @staticmethod
    def _break_parquet_files_into_groups(
        ...
        # for simplicity we just load the first file and get the number of rows
        ...
        max_files_per_group = int(cudf_max_num_elements / avg_num_rows)
        max_files_per_group = max(1, max_files_per_group)  # Ensure at least 1 file per group

        # Break the flat list of files into groups
        groups = [files[i : i + max_files_per_group] for i in range(0, len(files), max_files_per_group)]

        logger.info(
            f"Broke {len(files)} files into {len(groups)} groups with max {max_files_per_group} files per group"
        )
        return groups
